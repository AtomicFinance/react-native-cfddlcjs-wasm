...
function callCfddlc(modId, callId, fn, args) {
...
@objc(CfddlcjsWasm)
class CfddlcjsWasm: NSObject, WKScriptMessageHandler {
    ...
    override init() {
        ...
        let wasmFilePath = resourceBundle.url(forResource: "cfddlcjs_wasm", withExtension: "wasm")
        var bytes = [UInt8]()
    ...
    @objc
    func initCfddlc(_ modId: NSString, resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) -> Void {
    ...
    @objc
    func callCfddlc(_ modId: NSString, funcName name: NSString, arguments args: NSString, resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) -> Void {
        ...
        DispatchQueue.main.async {
            self.webView.evaluateJavaScript("""
            callCfddlc("\(modId)", "\(callId)", "\(name)", \(args));
            """) { (value, error) in
                if error != nil {
                    self.asyncPool.removeValue(forKey: modId as String)
                    reject("error", "\(error)", nil)
